...
export function publishPlaid(s:any,item:any,accountId?:string){
 ...
 for(const [remote,local] of Object.entries(item.mapping) as [string,string][]){
  if(accountId&&local!==accountId)continue;assert(state.accounts.some(a=>a.id===local&&!a.archived),'Mapped account is unavailable. Update bank settings.');const rows:any[]=[];
...
export function registerPlaid(app:any,store:Store,transport?:any){
 ...
 app.post('/api/v1/plaid/sync',locked(async(req:any,check:any)=>{const all=items(),item=all.find(i=>req.body?.accountId?Object.values(i.mapping).includes(req.body.accountId):i.itemId===req.body?.itemId);assert(item,'No bank connection is mapped to this account. Open Settings & backup, then Bank connections.');assert(Object.keys(item.mapping).length,'Map bank accounts first.');const result=await collectSync(call,item.accessToken,item.cursor);check();for(const {t,removed} of result.changes){const old=item.transactions[t.transaction_id];item.transactions[t.transaction_id]={t:removed?{...old?.t,...t}:t,removed:!!removed};}item.cursor=result.cursor;item.lastSync=new Date().toISOString();let count=0;store.transaction(()=>{const result=publishPlaid(store.state(),item,req.body?.accountId);count=result.count;store.save(result.state);vault.set('items',all);});return {count,sessionId:store.state().reconciliations.filter(r=>r.status==='OPEN'&&(!req.body?.accountId||r.accountId===req.body.accountId)&&r.notes?.startsWith('Plaid bank sync')).at(-1)?.id};}));
