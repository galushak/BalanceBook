...
test('private encrypted tokens, mapping checks, cursor rollback and API sync retries',async()=>{
 ...
 registerPlaid(app,store,async(path,body)=>{
  ...
  if(path==='/accounts/get')return {accounts:[{account_id:'remote',name:'Checking',type:'depository',balances:{current:90,iso_currency_code:'USD'}}]};
  if(path==='/transactions/recurring/get')return {inflow_streams:[],outflow_streams:[]};
 ...
 try{
  ...
  const active=store.state().accounts.find(a=>a.details?.plaid);assert.ok(active);assert.equal((await post('map',{itemId:'bank',mapping:{remote:'a'}})).statusCode,400);
  await post('disconnect',{itemId:'bank'});assert.equal((await app.inject('/api/v1/plaid')).json().items.length,0);assert.equal(store.state().events.length,1);
