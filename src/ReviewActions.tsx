...
export function ReviewActions({state:s,session,item,updateItem,add,openTx,l}:any){
 ...
 const addition=()=>{setMore(false);const merchant=suggestMerchant(item.description,s.payees,learnedAliases(s),session.accountId);add({returnPage:'reconcile',template:{notes:item.description,type:item.amount<0?'EXPENSE':'INCOME',accountId:session.accountId,amount:Math.abs(item.amount),date:item.date,payeeText:merchant.name,payeeId:merchant.payeeId},onSaved:async eid=>updateItem(item,{status:'LINKED',linkedTransactionId:eid})})};
