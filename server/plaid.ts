...
export function registerPlaid(app:any,store:Store,transport?:any){
 ...
 app.post('/api/v1/plaid/accounts',locked(async(req:any,check:any)=>syncOne(req.body?.itemId,check)));
 ...
 const syncOne=async(itemId:string,check:any=()=>{})=>{const all=items(),item=all.find(i=>i.itemId===itemId);assert(item,'Connection not found.');const result=await collectSync(call,item.accessToken,item.cursor);const accounts=await call('/accounts/get',{access_token:item.accessToken});
  try{const recurring=await call('/transactions/recurring/get',{access_token:item.accessToken});item.streams=[...(recurring.inflow_streams||[]),...(recurring.outflow_streams||[])];item.recurringError=null;}catch(e:any){item.recurringError=e.message;}
  check();for(const {t,removed} of result.changes){const old=item.transactions[t.transaction_id];item.transactions[t.transaction_id]={t:removed?{...old?.t,...t}:t,removed:!!removed};}item.cursor=result.cursor;item.lastSync=new Date().toISOString();let count=0;store.transaction(()=>{const s=store.state();importAccounts(s,item,accounts.accounts);const result=importTransactions(s,item);count=result.count;store.save(result.state);vault.set('items',all);});return {count,recurringError:item.recurringError};};
 ...
 app.post('/api/v1/plaid/disconnect',locked(async(req:any,check:any)=>{const all=items(),item=all.find(i=>i.itemId===req.body?.itemId);assert(item,'Connection not found.');await call('/item/remove',{access_token:item.accessToken});check();store.transaction(()=>{const s=store.state();for(const a of s.accounts.filter(a=>a.details?.plaid?.itemId===item.itemId)){const current=a.details.plaid.current;const outstanding=s.events.filter(e=>e.status==='ACTIVE'&&!e.bank?.refs?.some(r=>r.accountId===a.id&&!r.pending)).reduce((n,e)=>n+e.entries.filter(v=>v.accountId===a.id).reduce((m,v)=>m+v.amount,0),0);const net=s.events.filter(e=>e.status==='ACTIVE').reduce((n,e)=>n+e.entries.filter(v=>v.accountId===a.id).reduce((m,v)=>m+v.amount,0),0);a.opening=current+outstanding-net;delete a.details.plaid;a.version++;}store.save(s);vault.set('items',all.filter(i=>i.itemId!==item.itemId));});return {ok:true};}));
}
