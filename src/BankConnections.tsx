 ...
 async function connect(itemId?:string,resume=false){
  setBusy(true);setError('');try{
   ...
   const handler=(window as any).Plaid.create({token,...(resume?{receivedRedirectUri:location.href}:{}),onSuccess:async(publicToken:string)=>{
    handler.destroy();clearReturn();await run(async()=>{if(!update)await api('/plaid/exchange',{publicToken});const d=await api('/plaid');for(const i of d.items.filter(i=>!i.accounts.length||i.itemId===update))await api('/plaid/accounts',{itemId:i.itemId});l.setNotice('Bank connected. Map its accounts below, then sync transactions.');});
