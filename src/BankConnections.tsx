...
export function BankConnections({l}:any){
 ...
 async function connect(itemId?:string,resume=false){
  setBusy(true);setError('');try{
   ...
   const handler=(window as any).Plaid.create({token,...(resume?{receivedRedirectUri:location.href}:{}),onSuccess:async(publicToken:string)=>{
    handler.destroy();clearReturn();await run(async()=>{if(!update)await api('/plaid/exchange',{publicToken});const d=await api('/plaid');for(const i of d.items.filter(i=>!i.accounts.length||i.itemId===update))await api('/plaid/accounts',{itemId:i.itemId});l.setNotice('Bank connected. Map its accounts below, then use Pull from Plaid in Reconciliation.');});
 ...
 return <><Button onClick={()=>setOpen(true)}>Bank connections</Button>{open&&<Modal title="Bank connections" wide onClose={()=>{if(!busy)setOpen(false)}}><p>Connect a bank, map its accounts, then sync posted transactions into reconciliation. Missing transactions need your review. Balances are never overwritten.</p>{error&&<div role="alert" className="banner error">{error}</div>}{data&&<>
 ...
 {data.items.map(item=><section key={item.itemId} className="panel"><h3>{item.accounts[0]?.name||'Connected bank'}</h3><p>Last sync: {item.lastSync?new Date(item.lastSync).toLocaleString():'Not synced yet'}</p>{item.accounts.map(a=><Field key={a.id} label={`${a.name} · ${a.mask||a.subtype||a.type}`}><Select disabled={busy||!!item.mapping[a.id]} value={maps[item.itemId]?.[a.id]||''} onChange={e=>setMaps({...maps,[item.itemId]:{...maps[item.itemId],[a.id]:e.target.value}})} options={[{value:'',label:'Do not import'},...l.state.accounts.filter(a=>!a.archived).map(a=>({value:a.id,label:a.name}))]}/></Field>)}<Button disabled={busy} onClick={()=>run(()=>api('/plaid/accounts',{itemId:item.itemId}))}>Reload bank accounts</Button><Button disabled={busy} onClick={()=>run(()=>api('/plaid/map',{itemId:item.itemId,mapping:maps[item.itemId]||{}}))}>Save account mapping</Button><Button disabled={busy} onClick={()=>connect(item.itemId)}>Reconnect bank</Button><Button disabled={busy} onClick={async()=>{if(await l.confirm('Disconnect this bank from Plaid? Imported reviews and ledger transactions will remain.'))void run(()=>api('/plaid/disconnect',{itemId:item.itemId}));}}>Disconnect</Button></section>)}
 </>}</Modal>}</>;
}
