...
export interface Rule extends Entity {label:string;template:Partial<Tx>;start:string;unit:'days'|'weeks'|'months'|'years';interval:number;end?:string;count?:number;mode:'FIXED'|'VARIABLE';status:'ACTIVE'|'PAUSED'|'ENDED';notifyDays:number;autoSplit:boolean}
...
export interface Settings {version?:number;timezone:string;timeout:number;autoPost:boolean;forecastTarget:'month'|'date'|'rule';forecastDate:string;forecastRule:string;backupHour:number;retention:number;demo:boolean;theme:'system'|'dark'|'light'}
